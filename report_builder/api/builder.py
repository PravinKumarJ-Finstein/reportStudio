...
def _mirror_name_for(doc) -> str:
	# Deterministic, unique: "Title (RPT-XXXXX)". Title is sanitized so it
	# can't break SQL identifiers. The doc.name suffix guarantees uniqueness.
	title = (doc.title or doc.name).strip()
	# Drop characters likely to break Frappe URL/name handling.
